...
import SlowMovingStockView, { SlowMovingItem } from "./SlowMovingStockView";
...
export default function SlowMovingStockContainer() {
  ...
  const [slowStockValue, setSlowStockValue] = useState(0);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then(res => {
        ...
        setSlowStockValue(res.data?.slowStockValue ?? 0);
      })
      .catch(() => {
        setSlowMoving([]);
