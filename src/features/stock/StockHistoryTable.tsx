  ...
  return (
    <div className="flex flex-col h-full font-sans">
      ...
      <div className="mb-4 sm:mb-6 flex items-center gap-3 sm:gap-4">
        <div className={`h-10 w-10 sm:h-12 sm:w-12 rounded-xl sm:rounded-2xl ${bgAccent} ${accentColor} flex items-center justify-center shadow-sm border border-white/50`}>
        ...
        <div>
          ...
          <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1 sm:mt-1.5">
            {subtitle}
          </p>
        </div>
      </div>

      <div className="flex-1 bg-white rounded-2xl sm:rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
        {/* TABLE HEADER - Hidden on Mobile */}
        <div className="hidden sm:grid grid-cols-12 gap-4 px-6 py-4 bg-slate-50/50 border-b border-slate-100 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">
        ...
        {rows.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center py-10 sm:py-20 px-6 text-center">
            <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl sm:rounded-3xl bg-slate-50 text-slate-200 flex items-center justify-center mb-3 sm:mb-4">
          ...
          >
            {rows.map((row) => (
              <motion.div
                ...
                "
              >
                {/* Mobile Top Row: Date + Identifier */}
                <div className="flex sm:hidden items-center justify-between">
                  ...
                  <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-slate-100 text-slate-500">
                    <Hash size={10} />
                ...
                <div className="flex sm:hidden items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-slate-900 capitalize truncate max-w-[150px]">
                        {row.name}
                      </p>
                  ...
                  <div className="text-right">
                    <span className={`block text-sm font-black tabular-nums ${accentColor}`}>
                      ₹{(row.quantity * row.price).toLocaleString()}
                    </span>
                    <span className="block text-[9px] font-bold text-slate-400 tabular-nums">
                      @ {row.price}
                    </span>
                  </div>
                </div>
